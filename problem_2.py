        ...
        if filename.endswith(".csv"):  # will only read csv files
            ...
            with open(file_path, mode="r", encoding="utf-8") as file:  # i.e in [1]
                ...
                for row in reader:
                    station_name = row[
                        "STATION_NAME"
                    ]  # i.e in [4] # put the station names into an array
                    temperatures = []
                    for month, value in row.items():
                        if month not in {
                            ...
                            "LAT",
                            "LON",
                        }:  # we only need the station name and the temperature values, no need for other values
                            try:
                                temperatures.append(
                                    float(value)
                                )  # if the values cannot be converted into float, that means these are station names
                            except (
                                ValueError
                            ):  # when we find values that can be converted into floats , we put them in temps
                                pass
                ...
                for key, value in station_wise_temps.items():
                    temp_difference[key] = {
                        "max_temperature": round(
                            max(value), 2
                        ),  # dictionary of dictionary to hold the max
                        "min_temperature": round(min(value), 2),  # min
                        "temperature_difference": round(
                            max(value) - min(value), 2
                        ),  # difference of max and min rounded by 2
                    }  # i.e in [4]

                temperature_range = (
                    {}
                )  # holds the final result : The maximum range and the min,max temperature of the station
                ...
                with open(
                    "largest_temp_range_station.txt", "w", encoding="utf-8"
                ) as file:
                    file.write(
...
def temperature_stability():
        ...
        if filename.endswith(".csv"):  # will only read csv files
            ...
            with open(file_path, mode="r", encoding="utf-8") as file:  # i.e in [1]
                ...
                for row in reader:
                    station_name = row[
                        "STATION_NAME"
                    ]  # put the station names into an array
                    temperatures = []
                    for month, value in row.items():
                        if month not in {
                            "STATION_NAME",
    ...
    for key, value in station_wise_temps.items():
        mean_temperature[key] = {
            ...
            ),  # dictionary of dictionary to hold the mean of each station
        }  # i.e in [8]
    for temperatures, mean_temperature in mean_temperature.items():
        station = mean_temperature["station_name"]
        all_temperatures = mean_temperature[
            "temperatures"
        ]  # only the temperatures value keeping in a variable
    ...
    with open("temperature_stability_stations.tx", "w", encoding="utf-8") as file:
        file.write(
            "Most Stable:Station:"
            f"{min_std["station_name"]}: StdDev {min_std['std']}\u00b0C Most Variable:Station:{max_std["station_name"]}: StdDev {max_std['std']}\u00b0C"
        )  # i.e in [6]

...
class ImageModel:
    ...
    def open(self, path):
        ...
        self.original = img.copy()  # [5]
        self.current = img.copy()  # [5]
        self.undo_stack = []
        self.redo_stack = []
        self.path = path
    ...
    def get_details(self):
        if self.current is None:
            return
        height, width = self.current.shape[:2]  # [5]
    ...
    def set_current(self, img):
        if img is None or self.current is None:
            return
        self.undo_stack.append(self.current.copy())  # [5]
        self.current = img.copy()  # [5]
        self.redo_stack = []
    ...
    def blur(self, img, intensity):
        ...
        if intensity % 2 == 0:
            intensity += 1
        return cv2.GaussianBlur(img, (intensity, intensity), 0)  # [19]
    ...
    def brightness(self, img, beta):
        ...
        if beta > 100:
            beta = 100
        return cv2.convertScaleAbs(img, alpha=1, beta=beta)  # [8]
    ...
    def contrast(self, img, alpha):
        ...
        if alpha > 3.0:
            alpha = 3.0
        return cv2.convertScaleAbs(img, alpha=alpha, beta=0)  # [8]
    ...
    def rotate(self, img, angle):
        if img is None:
            raise ValueError(f"No image open")
        if angle == 90:
            return cv2.rotate(img, cv2.ROTATE_90_CLOCKWISE)  # [9]
        if angle == 180:
            return cv2.rotate(img, cv2.ROTATE_180)  # [9]
        if angle == 270:
            return cv2.rotate(img, cv2.ROTATE_90_COUNTERCLOCKWISE)  # [9]
    ...
    def resize(self, img, width=None, height=None):
        ...
        img_height, img_width = img.shape[:2]  # [5]
    ...
    def reset(self):
        if self.original is None:
            raise ValueError(f"No image open")
        self.undo_stack.append(self.current.copy())  # [5]
        self.current = self.original.copy()  # [5]
        return self.current.copy()  # [5]
